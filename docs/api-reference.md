...
| `POST` | `/api/v1/constraints/validate` | Validate CEL constraint expressions server-side (dry-run): `{constraints: [{name, expression},...], scope?: {allowed_paths?, forbidden_paths?}}` → `{valid: bool, results: [{name, valid, error?},...]}` — compiles each expression with the real CEL parser and validates scope glob-to-CEL conversion (authorization-provenance §7.6, TASK-007) |
