<script>
  ...
  // View spec generated by LLM — drives canvas node-type filter
  let viewSpecNodeTypes = $state(null); // string[] | null

  function applyViewSpec(viewSpec) {
    const data = viewSpec?.data;
    viewSpecNodeTypes = null;
    if (!data) return;
    if (data.concept) {
      // Use the existing concept-search path so conceptFilterIds drives the canvas
      conceptQuery = data.concept;
      doConceptSearch(data.concept);
    } else if (Array.isArray(data.node_types) && data.node_types.length > 0) {
      viewSpecNodeTypes = data.node_types;
    }
  }
        ...
        for (const block of blocks) {
          ...
          if (eventType === 'error') {
            ...
            try {
              ...
              if (parsed.view_spec) applyViewSpec(parsed.view_spec);
            } catch {
