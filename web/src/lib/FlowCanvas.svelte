<script>
  ...
  function drawWebGL(particles) {
    ...
    // Clean up buffers to prevent GPU memory leak on every frame
    gl.deleteBuffer(posBuf);
    gl.deleteBuffer(colBuf);
  }

  // Parse a CSS hsl() color string to RGBA (0-1 range).
  // particleColor() always produces hsl(h, s%, l%) strings.
  function cssColorToRGBA(color) {
    const match = color.match(/hsl\(\s*(\d+),\s*(\d+)%,\s*(\d+)%\s*\)/);
    if (!match) return { r: 0.23, g: 0.51, b: 0.96, a: 1 };
