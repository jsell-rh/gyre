...
- [-] [process-revision-complete] **F1: Response field name mismatch — `target_repo_id` used for dependent's repo.**  
...
- [-] [process-revision-complete] **F2: Aspirational test name — "dims non-highlighted edges" tests no edge dimming.**  
...
- [-] [process-revision-complete] **F3: No component tests for SpecDashboard or DetailPanel impact features.**  
...
- [-] [process-revision-complete] **F4: DetailPanel `loadImpactAnalysis` fetches `directDeps` but excludes `extends` links.**  
