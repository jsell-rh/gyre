...
- [-] [process-revision-complete] **F1 — All 5 query methods in `ChainAttestationRepository` adapter lack `tenant_id` filtering — spec-mandated tenant isolation violation.**  
...
- [-] [process-revision-complete] **F2 — `find_active_by_identity` does not filter by `expires_at` — returns expired key bindings as "active".**  
...
- [-] [process-revision-complete] **F3 — Missing `workspace_id` index on `chain_attestations` table per spec §5.3.**  
