...
- [ ] **F1 — `ConstraintViolation` event payload missing spec-required `attestation_id` field**
...
- [ ] **F2 — `ConstraintViolation` notification priority is 3 but spec requires 2**
...
- [ ] **F3 — Push-time constraint evaluation hardcodes `default_branch` to `"main"`**
