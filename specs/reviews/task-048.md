...
- [ ] **F1: `target_artifact` set to `source_artifact` in `reconcile_dependencies` — wrong data stored on all new edges.**
...
- [ ] **F2: Reconciliation incorrectly orphans edges from dependency files not changed in the current push.**
...
- [ ] **F3: `go.mod` detection uses `replace` directives instead of spec-required `require` directives.**
