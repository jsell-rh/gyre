...
- [ ] **F1 — `gyre briefing --workspace` is required; spec says optional.**  
...
- [ ] **F2 — `gyre inbox` requires `list` subcommand; spec defines flat command.**  
...
- [ ] **F3 — `gyre explore` fails when neither `--repo` nor `--workspace` is provided.**  
...
- [ ] **F4 — `gyre explore --repo` accepts a repo ID; spec says `--repo <name>`.**  
...
- [ ] **F5 — `gyre spec assist` requires `--repo-id` absent from spec signature.**  
...
- [ ] **F6 — `gyre explore --workspace` concept filter is silently ignored by server.**  
