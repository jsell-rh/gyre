...
- [ ] **F1 — `onSpanSelect` is a no-op stub; canvas highlighting never occurs.**  
...
- [ ] **F2 — No test for expanded span detail (attributes, input/output summary).**  
...
- [ ] **F3 — Dead code in test `shows no evaluative tab when no spans match the node`.**  
