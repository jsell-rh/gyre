...
- [ ] **F1: Silent policy creation error swallowing in `create_workspace` — `let _ =` on async operation.**
...
- [ ] **F2: Workspace creation policy seeding is not atomic with workspace creation.**
...
- [ ] **F3: Double workspace write — `apply_trust_transition` and `workspaces.update` both write `trust_level`, creating a partial-update failure window.**
...
- [ ] **F4: No test for the 409 error path on failed trust transition.**
