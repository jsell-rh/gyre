...
- **Spec-type simplification drift:** When a task creates domain types defined in a spec (especially in `gyre-common`), verify that each type matches the spec's definition **field-by-field**. Two common failure modes: (a) **array element simplification** — the spec defines `field: StructuredType[]` (e.g., `persona_constraints: PersonaRef[]` where `PersonaRef` has a `.name` property) but the implementation uses `Vec<String>`, which serializes as `["security"]` instead of `[{"name": "security"}]`. This compiles and passes unit tests but breaks all downstream code that accesses fields on array elements (e.g., CEL evaluators evaluating `input.persona_constraints[0].name`). (b) **Reference type substitution** — the spec says `field: FullType` (e.g., `key_binding: KeyBinding` with 8 fields) but the implementation embeds a 2-field reference/summary struct. This breaks the spec's self-containment property — offline verification and recursive chain validation require the full data to be embedded, not behind a repository lookup. For each domain type the task creates: open the spec section that defines it, list every field (name, type, cardinality), then open the implementation and verify each field exists with the correct type. Pay special attention to fields whose spec type is a struct — if the implementation uses `String`, `Id`, or a minimal ref type instead, that is a finding.
