...
22. **Response completeness — exhaustive field and section consumption:** When implementing a CLI command or display function that renders data from a server response, verifying field *names* (checklist item 21) is necessary but not sufficient — you must also verify field *coverage*:
    ...
    - **Item struct field coverage:** Vec<T> element types (e.g., `BriefingCompletedAgent`, `BriefingItem`) and direct response types (e.g., `NotificationResponse`) each contain multiple pub fields. After implementing a display function, enumerate ALL pub fields of each consumed struct — not just the Vec sections of the parent response. For each field, confirm your display code references it as a string literal or has an exclusion comment. Run `scripts/check-response-consumption.sh` — checks 4 and 5 detect missing item-level and response-level scalar fields mechanically. A display function that renders `agent_id`, `spec_ref`, `decisions`, and `uncertainties` from `BriefingCompletedAgent` but omits `conversation_sha` and `completed_at` is silently dropping two user-relevant fields.
