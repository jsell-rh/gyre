...
1. **Cross-Workspace section:**
   - Query `SpecLinkRepository` for inbound cross-workspace links to specs in this workspace
   - For each linked spec, check if it was modified since the briefing `since` timestamp
   - Check `Notification` table for `CrossWorkspaceSpecChange` notifications since `since`
   - Populate each entry: `source_workspace_slug`, `spec_path`, `summary` (description of the change)
...
3. **Wire data sources:**
   - Inject `SpecLinkRepository` (or relevant port) into the briefing handler
...
3. Read `crates/gyre-ports/src/spec_ledger_repo.rs` (or similar) for spec link queries
