...
- **Versioned in git** alongside other specs — changes are tracked and diffable. Prompt templates are in `specs/prompts/` which is in `spec-lifecycle.md`'s `ignored_paths` — prompt iteration should be fast and not require formal spec approval. The git SHA is recorded for audit, not for approval gating. **Editing via UI:** If a user edits a prompt template through the `specs/save` endpoint, the save handler detects the `specs/prompts/` path and skips the approval MR flow — the commit goes directly to the default branch (no feature branch, no MR, no approval notification). The ABAC check for prompt saves uses `action_override: "generate"` (not `write`) so that Supervised trust's `trust:require-human-mr-review` policy (which targets `write`) does not block prompt iteration. This matches the `ignored_paths` intent. Agent git pushes to `specs/prompts/` also skip approval gates because `spec-lifecycle.md`'s `ignored_paths` prevents task creation and approval invalidation — the consistency is maintained at both the UI and git-push levels.
