...
What changed since last visit, at the structural level. Powered by architectural deltas. The "since last visit" timestamp is obtained from the briefing response's `since` field (which reflects `last_seen_at`). The client caches this value and passes it to `GET /repos/:id/graph/timeline?since=<last_seen_at>` for the Change View query.
...
| 2 | **Spec pending approval** | Spec registry | Approve / Reject (inline, read spec content). Reject calls `POST /api/v1/specs/:path/reject` (new endpoint — sets spec status to `Rejected` with a `reason` field, closes the associated MR if the spec came from a `spec-edit/*` branch). |
