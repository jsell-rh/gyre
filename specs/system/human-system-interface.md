...
The server resolves the composite path to a specific spec in a specific repo. Resolution is always scoped to the **calling user's tenant** (the server uses the authenticated user's `tenant_id` to scope the workspace slug lookup, preventing cross-tenant resolution). The first segment is the **workspace slug** (URL-safe identifier, unique per tenant, as defined in `platform-model.md`'s Workspace struct). Same-repo links use just the spec path (existing behavior). Cross-repo same-workspace links use `repo_name/spec_path`. Cross-workspace links use the full `workspace_slug/repo_name/spec_path`.
...
| `abac-policy-engine.md` §"Action attributes" | Add `generate` action (used by `explorer-views/generate` endpoint — distinct from `write` to allow policies that permit LLM generation while restricting CRUD). |
