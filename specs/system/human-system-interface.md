...
| 4 | Cross-workspace spec change | Synchronous in spec lifecycle push handler (the push hook must query `spec_links` for inbound cross-workspace links and create notifications for dependent workspace members — amend `spec-lifecycle.md` to add this notification step alongside the existing task creation). The notification's `repo_id` is set to the *dependent* repo (the one that has the inbound link), not the changed repo. |
