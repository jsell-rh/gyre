...
**Mapping to MCP prompts:** `platform-model.md` §4 defines four MCP prompts (`system://persona`, `system://protocol`, `system://context`, `system://constraints`). Under this spec's model: `system://persona` delivers the assembled meta-spec prompt set (required + spec-level bindings). `system://protocol` delivers the protocol section. `system://context` delivers the task context. `system://constraints` delivers budget limits and operational parameters from the persona's operational fields. The four MCP prompts remain the delivery mechanism — the three-section structure above describes what goes INTO them.
...
| `platform-model.md` §2 Persona | The `Persona` struct gains `version: u32`, `required: bool` fields from this spec. Existing fields (`prompt`, `content_hash`, `slug`, `capabilities`, `protocols`, `model`, `temperature`, `max_tokens`, `budget`, `approval_status`, `approved_by`, `approved_at`) are preserved — `prompt` holds the prompt text, `content_hash` holds the SHA-256. The bootstrap persona list in `platform-model.md` §2 is superseded by the bootstrap table in this spec §2 (which also covers bootstrap step 5 in `platform-model.md` §8). The `/api/v1/personas` endpoints from `platform-model.md` §2 are **deprecated** in favor of `/api/v1/meta-specs?kind=meta:persona` — both endpoints remain functional during migration, but new code should use the meta-spec API. |
