...
1. New `version` number assigned, new `content_hash` computed from content
...
| `meta:persona` | `reconciliation` | Global | No | Re-implements specs under updated meta-specs (used by HSI §8 to exclude reconciliation deltas from divergence detection) |
