...
| M22: Workspace, Budget, Spec Links | In Progress | **M22.1** `Workspace` + `Persona` entities, `PersonaScope`, 12 new endpoints (`/workspaces`, `/personas`), `workspace_id` on repos. **M22.2** `BudgetConfig`/`BudgetUsage`, 3 budget endpoints, spawn 429 gate, daily reset job. **M22.3** `SpecLink`/`SpecLinkType`, 6 link types, forge enforcement, approval gates, 2 new endpoints (`/specs/{path}/links`, `/specs/graph`) |
