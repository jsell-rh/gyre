...
| M22: Workspace, Budget, Spec Links, Dep Graph, ABAC, Search | In Progress | **M22.1** `Workspace`+`Persona` entities, `PersonaScope`, 12 endpoints. **M22.2** `BudgetConfig`/`BudgetUsage`, 3 budget endpoints, spawn 429 gate. **M22.3** `SpecLink`/`SpecLinkType`, 6 link types, forge enforcement, approval gates. **M22.4** `DependencyEdge`, 6 cross-repo dep endpoints, Cargo.toml auto-detect. **M22.6** declarative ABAC `Policy` engine (8 operators, first-match-wins, default-deny), 5 policy endpoints. **M22.7** `SearchPort`/`MemSearchAdapter`, 2 search endpoints, `gyre_search` MCP tool |
