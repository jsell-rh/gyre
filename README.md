...
| M29: Workspace Integration | Done | Diesel tables for 9 platform entities, `workspace_id` filtering on agents/tasks/projects/repos/MRs API params, UI workspace selector wires all list views. **M29.5A** Migrate spec/gates/attestation stores to async port traits + Diesel adapters (migration 000009: quality_gates, gate_results, repo_push_gates, spec_policies, attestation_bundles, container_audit_records, spec_ledger_entries, spec_approval_events). **M29.5B** Migrate ABAC/compute/agent/workspace stores to KvJsonStore + BudgetRepository (migration 000008). |
| M30: Knowledge Graph Extraction | Done | **M30b** Push-triggered automatic polyglot knowledge graph extraction: on every push to the default branch, language-specific extractors run in background (git archive → AST parse → persist nodes/edges → record `ArchitecturalDelta`); supported languages: **Rust** (syn AST), **TypeScript/JavaScript**, **Python** (Flask/FastAPI endpoint detection), **Go** (tree-sitter); graph APIs return live data after push |
...
| M33: Project Removal | Done | `Project` entity removed from domain; Workspace is now the primary grouping entity for repos, tasks, and agents. All APIs migrated from `project_id` to `workspace_id`. |
...
| Incremental Graph | Done | Field-level diffs per commit in `/graph/timeline` (migration 000038); event→notification pipeline wires spec changes, gate failures, agent events to inbox automatically; flow particle visualization wired to real trace spans in Explorer |

1257 Rust + 434 vitest component + 50 Playwright E2E tests passing (including E2E Ralph loop integration test). Hexagonal architecture enforced mechanically. Migrations: 000001–000038.
