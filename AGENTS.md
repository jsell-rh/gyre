...
- **Agent List**: shows all registered agents with status. **"Spawn Agent" button** opens a modal to provision a new sub-agent (name, repo, task, branch, and **compute target** dropdowns — M24). On success, displays the agent token and clone URL for use by the spawned agent.
