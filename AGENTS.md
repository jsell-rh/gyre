...
| `GYRE_DATABASE_URL` | _(unset — in-memory)_ | SQLite database URL, e.g. `sqlite://gyre.db`. When set, all port traits persist to SQLite with WAL mode; Diesel migrations run automatically on startup. Unset = in-memory (default, stateless). (M10.1, M15.1) |
