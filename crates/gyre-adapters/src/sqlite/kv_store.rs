...
#[async_trait]
impl KvJsonStore for SqliteStorage {
...
#[async_trait]
impl BudgetUsageRepository for SqliteStorage {
    ...
    async fn increment_active(
        ...
        tokio::task::spawn_blocking(move || -> Result<BudgetUsage> {
            let mut conn = pool.get().context("get db connection")?;
            let ts = now_secs();
            // Upsert: create if missing, then increment atomically.
            let row = NewBudgetUsageRow {
                entity_key: &key,
                entity_type: &etype,
                entity_id: &eid,
            ...
            diesel::update(budget_usages::table.find(&key))
                .set((
                    budget_usages::active_agents.eq(
                        // max(0, active_agents - 1) via MAX() in SQLite
                        diesel::dsl::sql("MAX(0, active_agents - 1)"),
                    ),
                    budget_usages::updated_at.eq(ts),
                ))
                .execute(&mut *conn)
