...
impl MergeRequestRepository for SqliteStorage {
    ...
    async fn list_dependents(&self, mr_id: &Id) -> Result<Vec<Id>> {
        // Load all MRs and filter those whose depends_on contains mr_id.
        // For SQLite, JSON querying in SQL is complex; in-memory filter is simpler.
...
mod tests {
    ...
    async fn create_repo(s: &SqliteStorage, _workspace_id: &str, repo_id: &str) {
    ...
    #[tokio::test]
    async fn atomic_group_roundtrip() {
        let (_tmp, s) = setup();
        create_repo(&s, "p1", "r1").await;
        let mut mr = make_mr("mr1", "r1");
        mr.atomic_group = Some("group-alpha".to_string());
        MergeRequestRepository::create(&s, &mr).await.unwrap();
        let found = MergeRequestRepository::find_by_id(&s, &mr.id)
            .await
            .unwrap()
            .unwrap();
        assert_eq!(found.atomic_group.as_deref(), Some("group-alpha"));
    }
    ...
    async fn list_dependents_works() {
        ...
        let deps = MergeRequestRepository::list_dependents(&s, &Id::new("mr1"))
            .await
            .unwrap();
